...
def state(path, learner):
    with transaction(path, learner=learner) as db:
        ...
        return {
            ...
            "course": {"title": "Reliable agent execution", "episode_count": 4, "campaign": campaign_progress(db, learner)},
            "modes": MODES,
        }
...
def command(path, learner, action, body):
    ...
    with transaction(path, learner=learner, lock=True) as db:
        ...
        if action == "start":
            ...
            if mission_id:
                progress = {item["id"]: item for item in campaign_progress(db, learner)}
            ...
            if action == "submit":
                ...
                if parse_prediction(response["prediction"], count) is None or (diagnosis_required and not response["diagnosis"].strip()):
