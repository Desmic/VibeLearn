...
def manifest():
    paths = [*sorted((ROOT / "app").glob("*.py")), *sorted((ROOT / "web").rglob("*")), *sorted((ROOT / "db").glob("*.sql")), *sorted((ROOT / "design").glob("learning-design.json")), ROOT / "manage.py", ROOT / "requirements.lock"]
