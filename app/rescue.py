...
TRAINING = [
    dict(title='A reply lost in the storm', goal='Get one bridge gear. Not two.',
         brief='Pip sent an order. No reply came back. Does the workshop still remember our ticket?',
    ...
    dict(title='Pip wakes up with the wrong ticket', goal='Repair the courier without ordering twice.',
    ...
    dict(title='Someone changed the parcel', goal='Protect the original order and flag the changed request.',
    ...
    dict(title='The workshop forgot the ticket', goal='Locate the old gear without making another.',
    ...
    dict(title='A blank screen is not an empty book', goal='Keep uncertainty safe, then finish the missing delivery.',
    ...
    for m in moves:
        ...
        if isinstance(m,dict):
            ...
            s['feedback']={
                ...
                'workshop':'A gear turns behind the window. The workshop acted; Pip just did not get the reply.',
