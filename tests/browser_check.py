...
def main():
    ...
    with tempfile.TemporaryDirectory() as directory, sync_playwright() as playwright:
        ...
        try:
            ...
            expect(page.locator(".mission-chip").first).to_have_text("MISSION 01")

            # A browser AbortError must never leak its raw implementation message.
            page.evaluate("""() => {
                window.__vibelearnRealFetch = window.fetch;
                window.fetch = () => Promise.reject(new DOMException('signal is aborted without reason', 'AbortError'));
            }""")
            page.get_by_role("button", name="Start mission", exact=True).click()
            expect(page.locator("#notice")).to_contain_text("The connection took too long. Please try again.")
            expect(page.locator("#notice")).not_to_contain_text("signal is aborted")
            page.evaluate("window.fetch = window.__vibelearnRealFetch; delete window.__vibelearnRealFetch")
            checks.append("raw browser abort is normalized to a learner-facing retry message")

            page.get_by_role("button", name="Start mission", exact=True).click()
            expect(page.locator("#prediction")).to_be_visible()
            expect(page.locator(".mission-hud")).to_be_visible()
            expect(page.locator("#source")).to_be_disabled()
            assert not page.locator("#source-link").get_attribute("href")
            page.get_by_role("button", name="Lock in answer").click()
            ...
            expect(page.get_by_role("button", name="Save progress", exact=True)).to_be_visible()
            expect(page.locator(".save-progress-group")).to_contain_text("resume this run later")
            ...
            checks.append("visible Save progress persists the active run across browser reload")
            ...
            page.get_by_role("button", name="Save progress", exact=True).click()
            expect(page.locator("#save-status")).to_have_text("Saved to local database")
            checks.append("failed real request preserves progress across restart and reload")
            ...
            expect(page.locator(".stage-complete")).to_be_visible()
            ...
            other_page.get_by_role("button", name="Start mission", exact=True).click()
            ...
            page.get_by_role("button", name="Play another run").click()
