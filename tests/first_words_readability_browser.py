    ...
    with tempfile.TemporaryDirectory() as temp,sync_playwright() as p:
        ...
        try:
            ...
            for width in [360,390,430]:
                ...
                tray=page.locator('#controls').bounding_box();tools=page.locator('#world .game-view-tools').bounding_box()
                assert tools['y']+tools['height']<=tray['y'],(tools,tray)
                goal=page.locator('.mission').bounding_box()
                ...
                for selector in ['#goal','#actions button','#context']:
                    box=page.locator(selector).bounding_box();assert box['x']>=0 and box['x']+box['width']<=width and box['y']+box['height']<=844,(selector,box)
            print('200% text probe passed')
