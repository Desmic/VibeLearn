...
from tests.first_words_browser import action,generate
...
def main():
    with tempfile.TemporaryDirectory() as temp,sync_playwright() as p:
        ...
        try:
            ...
            action(page,'Connect the power lead');generate(page,'Generate from this input')
            ...
            for width in [360,390,430]:
                ...
                assert goal['height']<=844*.26,(goal,width)
                assert tray['height']<=844*.35,(tray,width)
                # Preserve a meaningful 3D band between objective and engine HUD.
                assert tray['y']-(goal['y']+goal['height'])>=110,(goal,tray,width)
                assert mast['height']<=75,(mast,width)
            ...
            print('200% text keeps bounded scrollable HUD and playable world')
