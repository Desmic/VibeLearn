...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp,sync_playwright() as p:
        ...
        try:
            ...
            choose(page,'Predict: the machine will say Moon')
            ...
            for prediction,hint in [('Moon',False),('Star',False),('Star',True)]:
                ...
                choose(q,f'Predict: the machine will say {prediction}')
            ...
            for width in (360,430,1280):
                ...
                choose(q,"Supply today's notice · “Moon route closed. The tower bell answers the five-point lantern mark.”")
                choose(q,'Predict: the machine will say Star');generate(q);complete_relay(q);action(q,'Finish Level 1 →','Level saved · practice recorded');expect(q.locator('#goal')).to_have_text('Mira heard you.',timeout=15000)
