...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp,sync_playwright() as p:
        ...
        try:
            ...
            checks.append('Tutorial gives one obvious action at a time and a guaranteed first rescue before normal failure.')

            action(page,'Continue with Zip →')
            expect(page.locator('#stage-name')).to_have_text('TOWER ROUTE · CHALLENGE')
            old=page.locator('.notice-marker[data-action="scan-moon"]');today=page.locator('.notice-marker[data-action="scan-star"]')
            expect(old).to_be_visible(timeout=10000);expect(today).to_be_visible(timeout=10000)
            with page.expect_response(lambda r:'/api/commands/' in r.url and r.request.method=='POST') as saved:old.click()
            assert saved.value.ok;expect(page.locator('#context')).to_contain_text('Old route')
            ...
            with page.expect_response(lambda r:'/api/commands/' in r.url and r.request.method=='POST') as saved:today.click()
            assert saved.value.ok;expect(page.locator('#context')).to_contain_text('five-point lantern mark')
            generate(page);until(page,'()=>!FirstWordsReview.runtime.world.animating')
            ...
            for width in (360,430):
                ...
                choose(q,'List route signs','Today’s notice · “Moon route closed. The tower bell answers the five-point lantern mark.”')
            ...
            checks.append('360/430 reduced-motion players can follow the same tutorial and solve transfer through the accessible route-sign fallback.')
