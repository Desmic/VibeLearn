        ...
        try:
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    page.locator('#login-submit').click();expect(page.locator('#entry-status')).to_contain_text('Reference:');expect(page.locator('#login-submit')).to_be_enabled();self.assertLessEqual(page.evaluate('document.documentElement.scrollWidth'),390)
                    ...
                    page.locator('#login-email').fill('owner@example.test');page.locator('#login-password').fill('test-password');page.locator('#login-submit').click();page.wait_for_url('**/first-words',timeout=15000);expect(page.locator('#rgi-intro')).to_be_visible(timeout=20000);expect(page.locator('#rgi-world .vl-playcanvas-engine')).to_be_visible();self.assertEqual(page.locator('#rescue-game').count(),0);self.assertEqual(page.locator('svg').count(),0)
                    page.get_by_role('button',name='Skip opening',exact=True).click();expect(page.locator('#engine')).to_be_visible(timeout=15000);expect(page.locator('#stage-name')).to_have_text('TUTORIAL · MOVE');page.get_by_role('button',name='Skip control practice',exact=True).click();expect(page.locator('#engine')).to_be_hidden();expect(page.locator('#machine-toggle')).to_be_visible();expect(page.get_by_role('button',name='Connect the loose power lead',exact=True)).to_be_visible();page.reload();expect(page.get_by_role('button',name='Connect the loose power lead',exact=True)).to_be_visible(timeout=15000);expect(page.locator('#engine')).to_be_hidden();expect(page.locator('#rgi-intro')).to_have_count(0)
