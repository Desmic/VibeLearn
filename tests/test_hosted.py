        ...
        try:
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    page.get_by_role('button',name='Skip opening',exact=True).click();expect(page.locator('#engine')).to_be_visible(timeout=15000);expect(page.locator('#stage-name')).to_have_text('TUTORIAL · MOVE');page.get_by_role('button',name='Skip control practice',exact=True).click();expect(page.get_by_role('button',name='Connect the power lead',exact=True)).to_be_visible();page.reload();expect(page.locator('#engine')).to_be_visible(timeout=15000);expect(page.locator('#rgi-intro')).to_have_count(0)
