...
function render(){
  ...
  text('#goal',complete?'The tower is open.':!s.powered?'Bring back Zip’s voice.':s.status==='success'?(s.round===0?'Zip is free.':'The tower is open.'):s.status==='wrong'?'Wrong way.':s.round===0?'Choose Zip’s context.':'Find the tower route.');
  text('#detail',complete?'Zip is beside you. Look to the printing loft when you are ready.':!s.powered?'Connect the loose lead to the repair socket.':s.status==='success'?(s.round===0?'Your friend is back by your side.':'Zip is waiting at the tower steps.'):s.status==='wrong'?(s.round===0?'The engine followed its input. Check Zip’s Moon plaque.':'That context led elsewhere. Try another route board.'):s.hinted?'Use the current clue. Each new piece joins the next input.':s.round===1&&s.clue==='none'?'Three route boards disagree. Scan one into Zip’s engine.':s.round===1?'Predict from the context you chose.':'Generate now, or inspect Zip’s Moon plaque first.');
  ...
  else if(s.status==='wrong'){button(s.round===0?'Inspect Zip’s Moon plaque':'List the route signs',s.round===0?'scan-moon':'notices',s.round===0);}
  ...
  else if(s.round===1&&s.clue==='none')button('List the route signs','notices',false);
  ...
  else{button(s.pieces===0?'Make the first piece':s.pieces<4?'Make the next piece':'Speak to the gate →',s.pieces<4?'step':'send');if(s.round===1&&s.pieces===0)button('List the route signs','notices',false);}
...
function frame(){
  ...
  for(const marker of $('#markers').children){
    ...
    if(marker.dataset.action){
      marker.disabled=Boolean(blocked()||!s.available_actions?.includes(marker.dataset.action));
      marker.classList.toggle('chosen',marker.dataset.clue===s.clue);
    }
