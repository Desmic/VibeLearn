...
window.RescueStage = (() => {
  ...
  function attach(root, state, config, {look, sandbox, caseIndex = 0, autoPlay = false} = {}) {
    ...
    if (world && config.level < 6) {
      ...
      world.classList.add('rg-direct-play');
      ...
      const hint = document.createElement('span'); hint.className = 'rg-play-hint';
      hint.textContent = 'Inspect the scene, then choose Pip’s move'; world.append(hint);
      ...
      world.append(wrap);
      const observations = root.querySelector('.rg-observations');
      if (observations) observations.remove();
      const tools = root.querySelector('.rg-tools');
      if (tools) {
        tools.classList.add('rg-world-actions');
        tools.setAttribute('aria-label', 'Choose Pip’s next move');
        const label = document.createElement('strong'); label.className = 'rg-action-label';
        label.textContent = state.failed ? 'TRY ANOTHER IDEA' : 'YOUR MOVE';
        tools.prepend(label);
        world.append(tools);
      }
    }
...
window.RescueGame = (() => {
  ...
  function sync(busy,a){
    ...
    if(!busy&&!pending){attempt=a;setResponse(a.response.rescue);if(a.status==='draft'&&a.snapshot.rescue.level>=6)restorePlan(log.draft);render(a,a.snapshot.rescue.missions||[],cb);}
  }
