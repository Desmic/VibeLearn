...
def ingest(index:dict,raw_results:dict,execution_receipts:dict,native_requirements:dict|None=None,learning_design_sha256:str|None=None):
    ...
    for review_pass in PASS_ORDER:
        ...
        if raw is None:
            continue
        assignment=build_assignments(workspace,native_requirements,learning_design_sha256).get(review_pass)
        if not assignment:
...
def read_raw_results(root:Path):
    ...
    for path in sorted(root.rglob("*.json")):
        ...
        elif schema in ("vibelearn.critic-execution-receipt.v1", "vibelearn.critic-execution-receipt.v2"):
            review_pass=value.get("pass")
            if not isinstance(review_pass,str) or not review_pass:
...
def main():
    ...
    parser.add_argument("--native-requirements",type=Path,help="Exact native execution contracts used to build these assignments")
    parser.add_argument("--learning-design-sha256",help="Exact design digest from the original intent-comparison assignment")
    args=parser.parse_args()
    try:
        index=load(args.index)
        raw_results,execution_receipts=read_raw_results(args.results_root)
        native=load(args.native_requirements) if args.native_requirements else None
        workspace,normalized=ingest(index,raw_results,execution_receipts,native,args.learning_design_sha256)
    ...
    assignments=build_assignments(workspace,native,args.learning_design_sha256)
