...
- Critic evidence must match the claim: use live/browser interaction for physicality/tutorial/transition claims, motion over time for animation/cinematic claims, and actual listening for audio-quality claims when available. Screenshots/source alone cannot certify those dimensions. Read `docs/ART-WORLD-DIRECTION-CRITIC.md` and `docs/CRITIC-POLICY.md`.
