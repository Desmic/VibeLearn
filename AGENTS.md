...
- Run **story, art/world-direction, gameplay and learning** criticism as independent gates. Read `docs/ART-WORLD-DIRECTION-CRITIC.md` and `docs/CRITIC-POLICY.md`.
...
- Read `docs/AUTOMATED-DEVELOPMENT-SYSTEM.md`. Bounded integration of the existing Terminal PM Agent/orchestrator into VibeLearn is now approved. Start with the lean worker -> reviewer -> evidence loop; reviewer criticism implicitly includes attempting a valid proof/reproduction. Reuse donor execution/session/evidence/recovery ideas selectively rather than transplanting the full donor stack. This does not authorize a broad rewrite, role explosion, autonomous production deployment or Level 2 work.
...
- Preserve learner data, supplied designs and unrelated work. Test on disposable databases. No live-model integration, untrusted runner, paid resources or broad rollout without explicit authorization. Never fake live verification.
